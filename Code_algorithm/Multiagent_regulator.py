...
from FindOptimalContract import FindOptimalContract
from MultiagentDataProcess import MultiagentDataProcess
from PlotPie import PlotPie
from PlotLine import PlotLine
from MarketShareWarning import MarketShareWarning
from Contractid_usertype_pie import Contractid_usertype_pie
from Contractid_usertype_line import Contractid_usertype_line
from Contract_dynamic_price import Contract_dynamic_price
from Contract_price_line import Contract_price_line
from CustomerPriceDynamic import CustomerPriceDynamic
from PlotLine_profit import PlotLine_profit
...
class Person(Agent):
    ...
    def __init__(self, unique_id, model): 
        ...
        self.contract_number = random.randint(1, 5) # 定义用户的合同编号
    ...
    def communicate_contract(self, other_person):
        if self.contract_number != other_person.contract_number:
            ...
            optimal_contract, whetherchanged = FindOptimalContract(selfid,self_contract,neighbor_contract)
            if whetherchanged == 1:
...
def main():
    ...
    for i in range(iteration_time):  # Change this number to run more or fewer steps
        ...
        round_profit = Profit(round_data)
        # print(round_profit)
        ...
        contract_price.append(Contract_dynamic_price())
        print("\n")
    '''
    #######################################################################################    
        # #draw the plot
    ...
    CustomerPriceDynamic(contract_price)
    ...
    df_profit_list.to_csv('profit_list.csv')
    
    # print(customer_info)
    contract_market_number, contract_market_share, contractid_usertype_number, contractid_usertype_share = MultiagentDataProcess(contract_number, customer_info, numberofcluster=3)
    ...
    PlotPie(iteration_time, contract_market_share)
    # draw a line chart of contract market share
    PlotLine(contract_market_share)   
    
    # draw a pie chart of contractid and usertype
    Contractid_usertype_pie(iteration_time, contractid_usertype_share)
    # draw a line chart of contractid and usertype
    Contractid_usertype_line(contractid_usertype_share)
    
    # draw a line chart of contract price
    Contract_price_line(contract_price)
    
    # draw a line chart of profit
    PlotLine_profit(profit_list)
