...
3. 将所有生成的暂时用不到的数据，保存到一个文件夹中
'''
def MultiagentDataProcess_retailor(contract_number, data, numberofcluster):
    ...
    df_id_contract.to_csv('customer_id_contract_data_retailor.csv', index=False) # 将用户的id和其选择合同号保存为csv文件
    ...
    # save the data to csv
    df1.to_csv('contract_market_number_retailor.csv', index=False)
    df2.to_csv('contract_market_share_retailor.csv', index=False) 
    # contract market share and the contract market number of each contract in each round are stored in the csv file
    ...
    # save the data to csv
    df_contract1_usertype_number.to_csv('contract1_usertype_number_retailor.csv', index=False)
    df_contract1_usertype_share.to_csv('contract1_usertype_share_retailor.csv', index=False)
    df_contract2_usertype_number.to_csv('contract2_usertype_number_retailor.csv', index=False)
    df_contract2_usertype_share.to_csv('contract2_usertype_share_retailor.csv', index=False)
    df_contract3_usertype_number.to_csv('contract3_usertype_number_retailor.csv', index=False)
    df_contract3_usertype_share.to_csv('contract3_usertype_share_retailor.csv', index=False)
    df_contract4_usertype_number.to_csv('contract4_usertype_number_retailor.csv', index=False)
    df_contract4_usertype_share.to_csv('contract4_usertype_share_retailor.csv', index=False)
    df_contract5_usertype_number.to_csv('contract5_usertype_number_retailor.csv', index=False)
    df_contract5_usertype_share.to_csv('contract5_usertype_share_retailor.csv', index=False)
    
    #************************************************************************************************************************
    # 第三部分，将所有生成的暂时用不到的数据，保存到一个文件夹中
    # 创建一个文件夹
    # folder_name = 'MultiagentDataProcess_retailor_data'
    # if not os.path.exists(folder_name):
    #     os.makedirs(folder_name)
    
    # file_list = ['contract_market_number_retailor.csv', 'contract_market_share_retailor.csv', 'contract1_usertype_number_retailor.csv',
    #              'contract1_usertype_share_retailor.csv', 'contract2_usertype_number_retailor.csv', 'contract2_usertype_share_retailor.csv',
    #              'contract3_usertype_number_retailor.csv', 'contract3_usertype_share_retailor.csv', 'contract4_usertype_number_retailor.csv',
    #              'contract4_usertype_share_retailor.csv', 'contract5_usertype_number_retailor.csv', 'contract5_usertype_share_retailor.csv']
    # for i in file_list:
